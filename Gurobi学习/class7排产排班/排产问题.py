...
def BuildModel(Production, workingHours, workingHoursLimt, weighted1, weighted2, weighted3, bestX, bestY, bestE, bestU, bestV, bestW):
    xindex = {}    #第t天生产第i种产品的数量，实际存储的是1/该商品每小时的产量
    yindex = {}    #第t天是否生产第i种产品
    ...
    for t in range(7):
        ...
        Vindex[t] = 0
        Windex[t] = 0
        for i in range(len(Production)):#len(Production)=26 0~25
            xindex[i,t] = 1/Production[i][2]#订单i在t天的数量
            yindex[i,t] = 0#订单i在t天是否安排生产
            Nindex[i,t] = 0#一个随机的整数
            for j in range(len(Production)):
                #如果i不等于j，并且i,j属于同一类型的产品，eijt i在t天借j的料架数量
                if i!=j and Production[i][1] == Production[j][1]:
                    Eindex[i,j,t] = 0#初始化Eijt变量的数量
    ...
    try:
        ...
        #获得优化结果
        for i in xindex.keys():
            bestX[i] = x[i].x#获取x变量的值
            bestY[i] = y[i].x#获取y变量的值
...
try:
    ...
    weighted3 = 1                                   #班次所占的权重
    workingHours = 8                                #工作时长,每天单班正常工作的时长
